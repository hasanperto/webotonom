-- ============================================
-- Destek Sistemi - Departman Yapısı (Güvenli Versiyon)
-- Bu dosya mevcut yapıyı kontrol ederek sadece eksik olanları ekler
-- ============================================
...
-- ============================================
-- Tickets Tablosu Güncellemeleri (Güvenli)
-- ============================================

-- 1. Mevcut constraint'i kontrol et ve sil (eğer varsa)
...
-- 2. department_id kolonu ekle (eğer yoksa)
SET @col_exists = (
    SELECT COUNT(*) 
...
-- 3. message kolonu ekle (eğer yoksa)
SET @col_exists = (
    SELECT COUNT(*) 
    FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE()
...
SET @sql = IF(@col_exists = 0, 
    ...
    'SELECT "Column message already exists" AS info;'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 4. category kolonu ekle (eğer yoksa)
SET @col_exists = (
    SELECT COUNT(*) 
    FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE()
...
SET @sql = IF(@index_exists = 0, 
    ...
    'SELECT "Index department_id already exists" AS info;'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@constraint_exists = 0, 
    'ALTER TABLE `tickets` ADD CONSTRAINT `tickets_ibfk_department` FOREIGN KEY (`department_id`) REFERENCES `support_departments` (`id`) ON DELETE SET NULL;',
...
-- ============================================
-- Ticket Numarası Trigger (Güvenli)
-- ============================================

-- Mevcut trigger'ı kontrol et ve sil (eğer varsa)
...
BEGIN
    IF NEW.ticket_number IS NULL OR NEW.ticket_number = '' THEN
        SET NEW.ticket_number = CONCAT('TK-', DATE_FORMAT(NOW(), '%Y%m%d'), '-', 
            LPAD(
